...
1. **ความเรียบร้อยสำหรับผู้ใช้ 25–65+** (ไม่ต้องแก้ฐานข้อมูล)
   ...
   - ✅ (26 ก.ย. 69, commit `01df31b`) แถวภาพยนตร์ "ไทยและเอเชีย" กรองหนังอีโรติก/pink film ยุคเก่า (TMDB ไม่ได้ติดป้าย adult) ด้วย `vote_count.gte` 100 (`ORIGIN_MOVIE_MIN_VOTES`, เฉพาะภาพยนตร์ — ซีรีส์ยัง 5 เพราะละครไทยคนโหวตน้อย) + `without_keywords` (`ADULT_KEYWORD_IDS`) · ผล: หายหมดทั้งญี่ปุ่น เกาหลี (Adultery Alumni Association ฯลฯ) ไต้หวัน อินเดีย (Lust Stories 3) ฮ่องกง · ผลข้างเคียง: หนังใหม่มาก (ออกปี 2025–2026 ที่คนโหวตยังไม่ถึง 100) หลุดจากแถว เช่น ไทย "เส้นตาย สายลวง", "ปัง" — ถ้าพี่แอ้อยากได้คืน: ดึงเพิ่มอีกชุดสำหรับหนังออกฉาย 12 เดือนล่าสุดด้วยเกณฑ์โหวตต่ำกว่า
2. **หน้า `/search` โหมด EN** — `language` ยังคงที่ `th-TH` → ให้ตามปุ่ม ไทย | EN แบบหน้าแรก · ผลค้นหายังไม่ใช้ `applyEnglishTitles`
...
### 🧓 ความเรียบร้อยสำหรับผู้ใช้ทุกวัย (26 ก.ย. 69) — ✅ deploy และทดสอบบนเว็บจริงแล้ว (commit `a25ebde`, `46396a2`)
- `app/not-found.tsx` + `app/error.tsx` ใช้ `components/ErrorScreen.tsx` ร่วมกัน (ไทย/EN, ปุ่มสูง ≥48px, มี SiteFooter) · ทดสอบ: /no-such-page ตอบ 404 + "ไม่พบหน้านี้" · หน้า error ยังไม่ได้ทดสอบจริง (สั่งให้เกิดเองไม่ได้)
- แนะนำสำหรับคุณ: ถ้าไม่มีรายการ/ไม่ล็อกอิน/TMDB จับคู่ไม่ได้ (API ตอบ source `…(fallback)`) → `components/ForYouInvite.tsx` กล่องชวนบันทึก (ไม่ล็อกอิน → ปุ่ม "เริ่มใช้งานฟรี", ล็อกอินแล้ว → "เลือกเรื่องจากด้านล่าง") แทนแถวที่ซ้ำกำลังมาแรง · รอ `mediaChecked` ก่อนตัดสิน (กันกล่องกระพริบ) · ทดสอบแล้วกรณีไม่ล็อกอิน
- ชื่อเรื่องที่ TMDB ไม่มีชื่อไทย: `applyEnglishTitles` ใน `lib/tmdb.ts` (ภาษาต้นฉบับไม่ใช่ th/en และชื่อ = ชื่อต้นฉบับ → ดึงรายการเดิมเป็น en-US 1 ครั้ง ใช้ชื่ออังกฤษ) ใช้กับ grid หน้าแรก, trending, top_rated, for_you, origin, network · ผลบนเว็บจริง: แถวเกาหลี/ญี่ปุ่น/WeTV ไม่เหลือชื่ออักษรเกาหลี/ญี่ปุ่น/จีนเลย
- ⚠️ แถวตามช่อง (`fetchNetworkRow`) ต้องกรอง `READABLE_TITLE` ด้วยชื่อ th-TH **ก่อน** เปลี่ยนเป็นอังกฤษ — รอบแรกกรองทีหลัง รายการจีนที่ไม่ได้ฉายในไทยเลยกลับมา (哈哈哈哈哈 → "HaHaHaHaHa") · แก้แล้ว: WeTV → หอมรักมิรู้เลือน, ล่าหยก… เหมือนเดิม
...
- Watchlist: ลิงก์ "📖 นิตยสาร OnTheBridge" ใน `SiteFooter` (ค่าคงที่ `MAGAZINE_URL`) + เมนูข้างบนจอคอม + เมนู ☰ มือถือใน `AppShell` — ✅ พี่แอ้ทดสอบผ่าน 26 ก.ย. 69 (เมนูข้างจอคอม + เมนู ☰ มือถือ กดแล้วไปหน้านิตยสาร)
...
- **โดเมน — ✅ ตัดสินแล้ว 26 ก.ย. 69: `onthebridge.in.th` ซื้อที่ Z.com** (360 บาทปีแรก / ต่ออายุ 400 บาท ไม่รวม VAT; บุคคลทั่วไปใช้บัตรประชาชนได้ ต้องส่งเอกสารให้ Z.com ตรวจก่อนสั่งซื้อ) · พี่แอ้กำลังดำเนินการ จะแจ้งเมื่อเสร็จ · ตัดตัวเลือกอื่นเพราะ: .com/.net/.org มีคนจดแล้ว, onthebridgemag.com ยาวไป, .co แพง (~1,000–1,500 บาท/ปี), .online/.cc/.xyz มีสแปมเยอะ · เมื่อซื้อเสร็จ: เพิ่ม zone ใน Cloudflare (ฟรี) → พี่แอ้ใส่ nameserver 2 ตัวที่ Z.com → Claude ต่อ custom domain ให้ Pages (`onthebridge.in.th`) + Vercel (`watchlist.onthebridge.in.th`) → แก้ `SITE_URL` (2 ไฟล์นิตยสาร), `WATCHLIST_URL`, `MAGAZINE_URL`, `NEXT_PUBLIC_SITE_URL` → พี่แอ้แก้ Supabase Site URL + Redirect URLs → redirect ที่อยู่เดิม · ข้อมูลเดิม: นิตยสาร OnTheBridge อยู่ที่ https://onthebridge.pages.dev/ (Cloudflare Pages) · แผนที่คุยไว้: จดโดเมนผ่าน Cloudflare Registrar (ราคาทุน ~350–400 บาท/ปี — ขัดกติกา "ฟรีเท่านั้น" ต้องให้พี่แอ้อนุมัติเป็นข้อยกเว้น) → นิตยสาร = `onthebridge.xxx`, Watchlist = `watchlist.onthebridge.xxx` (CNAME → `cname.vercel-dns.com`, DNS only/เมฆเทา) · ต้องแก้ Supabase Site URL + Redirect URLs, `NEXT_PUBLIC_SITE_URL`, และ redirect จาก vercel.app เดิม
...
- **บทวิจารณ์ของเราเอง (ตัดสิน 26 ก.ย. 69):** เขียนในนิตยสาร หมวดใหม่ **F · Film · ภาพยนตร์ & ซีรีส์** (รีวิว + เบื้องหลังอุตสาหกรรมหนัง — เปิดขึ้นเว็บแล้ว ยังไม่มีบทความ, พี่แอ้จะค่อยๆ เขียน) · Watchlist จะแสดงแค่ป้าย "OnTheBridge แนะนำ" + ประโยคสั้น + ปุ่ม "อ่านบทวิจารณ์เต็ม ↗" + แถว "OnTheBridge แนะนำ" หน้าแรก โดยอ่านจาก `articles.json` ของนิตยสาร (ต้องเพิ่ม `| tmdb: movie/12345` ในบรรทัดแรกของไฟล์บทความ) — ทำพร้อมหน้าแยกของแต่ละเรื่อง หลังได้โดเมน · หลักการ: ต้องดูจริงก่อนเขียน, AI ช่วยเรียบเรียงได้แต่ต้องบอกผู้อ่าน, ไม่รับเงินรีวิว
